...
8.  Carry out initial internet check, do the following at a Linux terminal prompt.
    ...
    ```
       and edit the sections of that file *"config interface 'wan' "* and *"config device 'wan_eth1_dev' "*, then run command `reboot`, then SSH in again and recheck the google.com ping.  
    ```console
