 ...
 * Step 4 above for dd image writing to micro SD is basically the same in Mac as it is in Linux.  Hovever, in a quirk of the Mac nix system, small case unit should be used for the "BS=" flag.  See the script below I used.  I couldn't find any native app for resizing the ext4 rootfs partition to take up the full card if you're wanting to do that, but you can do this task by booting the excellent GParted live USB - [GParted Live USB](https://gparted.org/liveusb.php "GParted Live USB").
```console
iMac:[download folder] youruser$ dd if=openwrt-bcm27xx-bcm2711-rpi-4-[*complete the file name here*].img of=/dev/mmcblk0 bs=4~~M~~*m*; sync
```
