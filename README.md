...
<span style="color:red">UPDATE 20201118 - There appears to be some major dramas with the latest snapshots giving show-stopper kernel core-dump errors when building is attempted.  Stay tuned - I have logged an error report and will follow that through and update things here when resolved :)</span>.
...
1. By default LuCI is set up running on nginx-ssl webserver (rather than the usual vanilla uhttpd webserver based LuCI).
2. Render the new router as immediately accessible via wifi network SSID *"Rivendell"* and password *Hobbiton* to enable easy further setting up for individual use cases.
